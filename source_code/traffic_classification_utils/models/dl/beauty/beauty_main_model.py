__author__ = 'dk'
from models.dl.attacks import Beauty_model as CNN_model, parser_raw_data
from models.dl.beauty import cnn_model_config
...
class model(abs_model):
    def __init__(self, dataset, randseed, splitrate):
        super(model,self).__init__('beauty',randseed= randseed)
    ...
    def parser_raw_data(self):
        ...
        ##从原始数据集构建DF所需的数据集
        X_train,y_train, X_valid, y_valid, X_test, y_test = parser_raw_data(self, self.full_rdata, max_len = cnn_model_config.learning_params_template['input_length'],burstification=True)
    ...
    def train(self):
        ...
        cnn_model_config.nb_classes_template = num_class
        print(num_class)
        ...
        cnn_model = CNN_model(num_class = num_class)
        cnn_model.build_model()
        #cnn_model.model.summary()
        cnn_model.fit(X_train=X_train,y_train=y_train,
                     X_valid= X_valid, y_valid = y_valid,
                     batch_size= cnn_model_config.learning_params_template['batch_size'],
                     epochs=cnn_model_config.learning_params_template['epoch'])

        cnn_model.save_model(path=self.model)
        score = cnn_model.evaluate(X_test=X_test, y_test = y_test)
        print('[Beauty Test on {0} accuracy {1}'.format(self.dataset, score))
    def test(self):
        X_train,y_train, X_valid, y_valid, X_test, y_test = self.load_data()
        y_test = np_utils.to_categorical(y_test, num_classes= self.num_classes())
        X_test  = X_test[:, :,np.newaxis]

        cnn_model = CNN_model(num_class= self.num_classes())
        cnn_model.load_model(self.model)
        score = cnn_model.evaluate(X_test=X_test,y_test=y_test)
        print('Beauty Test on {0} accuracy :{1}'.format(self.dataset,score))
    ...
    def predict(self,pkt_size):
        ...
        if self.cnn_model == None:
            self.cnn_model = CNN_model(num_class= self.num_classes())
            self.cnn_model.load_model(self.model)

        x = [pad_sequence(_pkt_size, max_len= cnn_model_config.learning_params_template['in_dim']) for _pkt_size in pkt_size]
        x = np.array(x)[:, :,np.newaxis]
        y_logit = self.cnn_model.predict(x, actual_lable=True)
    ...
    def get_feature(self):
        ...
        cnn_model = CNN_model(num_class= self.num_classes())
        cnn_model.load_model(self.model)
        logit, feature = cnn_model.predict(X_test=X_test,actual_lable=False, return_feature=True)
...
if __name__ == '__main__':
  for test_rate in [0.1]:
    print(test_rate)
    dataset='app60'
    cnn_model = model(dataset, randseed= 128, splitrate=test_rate)
    #cnn_model.parser_raw_data()
    #cnn_model.train()
    cnn_model.test()
    print(dataset)
    print(test_rate)
