...
from models.dl.beauty.cnn_model_config import  learning_params_template
from keras import optimizers

def build_model(learn_params=learning_params_template, nb_classes= -1):
    ...
    if first_l["name"] == 'dropout':
        ...
        model.add(Conv1D(filters=first_l['filters'],
                         ...
                         input_shape=(learn_params['input_length'],1)))
    ...
    for l in rest_l:
        if l["name"] == 'maxpooling':
            ...
            if 'last' in l and  l['last'] == True:
                l['units'] = nb_classes
            print(l['units'])
            if l['regularization'] > 0.0:
                module=Dense(units=l['units'],
                               activation=l['activation'],
                            kernel_regularizer=regularizers.l2(l['regularization']),
                            activity_regularizer=regularizers.l1(l['regularization'])
                )
            else:
                module=Dense(units=l['units'], activation=l['activation'])
...
if __name__ == '__main__':
    import  numpy as np
    model = build_model(nb_classes=200)
    print(model.summary())